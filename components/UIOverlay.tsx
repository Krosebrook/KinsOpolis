    ...
    <div className="absolute bottom-full mb-4 left-1/2 -translate-x-1/2 w-56 bg-slate-900/95 backdrop-blur-md text-white rounded-2xl p-4 border-4 border-slate-700 shadow-2xl z-50 animate-fade-in pointer-events-none">
       ...
       <div className="space-y-1.5 bg-black/30 p-3 rounded-xl">
          {config.cost > 0 ? (
            ...
            <div className="text-center text-slate-400 font-bold text-sm italic">Free</div>
          )}

          {config.popGen > 0 && (
             <div className="flex justify-between items-center text-sm">
                <span className="text-slate-400 font-bold">Friends</span>
                <span className="text-blue-300 font-black">+{config.popGen} / day</span>
             </div>
          )}
          ...
          {config.incomeGen > 0 && (
             <div className="flex justify-between items-center text-sm">
                ...
                <span className="text-yellow-300 font-black">+${config.incomeGen} / day</span>
             </div>
          )}
       </div>
    </div>
...
const ToolButton: React.FC<{
  ...
  return (
    ...
    >
      ...
      {config.cost > 0 && (
        <span className={`text-[10px] md:text-xs font-black leading-none mt-0.5 bg-black/10 px-1.5 rounded-full ${canAfford ? 'text-green-700' : 'text-red-600'}`}>${config.cost}</span>
      )}
    </button>
  );
};
    ...
    <div className="absolute inset-0 pointer-events-none flex flex-col justify-between p-3 md:p-6 font-sans z-10 text-slate-800">
      ...
      <div className="flex flex-col md:flex-row md:justify-between md:items-start pointer-events-auto gap-4 w-full max-w-full">
        ...
        <div className="bg-white/95 p-3 md:p-4 rounded-3xl border-4 border-blue-400 shadow-[0_8px_0_rgb(59,130,246)] flex gap-4 md:gap-8 items-center justify-between md:justify-start w-full md:w-auto transform hover:scale-[1.02] transition-transform">
          ...
          <div className="flex flex-col items-center">
             <span className="text-2xl md:text-3xl">📅</span>
             <span className="text-sm md:text-xl font-black text-slate-700">Day {stats.day}</span>
          </div>
        </div>
        ...
        <div className={`w-full md:w-96 bg-indigo-600 text-white rounded-3xl border-4 border-indigo-400 shadow-[0_8px_0_rgb(99,102,241)] overflow-hidden transition-all ${!aiEnabled ? 'opacity-80 grayscale' : ''}`}>
          <div className="bg-indigo-800/50 px-4 py-2 flex justify-between items-center">
            <span className="font-black text-sm uppercase tracking-wider flex items-center gap-2">
              {aiEnabled ? (
                <>
                  <span className={`text-xl ${isGeneratingGoal ? 'animate-spin' : 'animate-bounce'}`}>🤖</span>
                  Robot Helper
                </>
              ) : (
                 "Sandbox Mode"
              ...
              currentGoal ? (
                <>
                  ...
                  <div className="flex justify-between items-center bg-black/20 p-3 rounded-xl">
                    ...
                    <div className="text-sm font-black text-green-300 bg-green-900/40 px-3 py-1 rounded-full border border-green-400/30">
                      +${currentGoal.reward} 🎁
                    </div>
                  </div>
                  ...
                  {currentGoal.completed && (
                    <button
                      ...
                      className="mt-3 w-full bg-green-500 hover:bg-green-400 text-white font-black py-3 px-4 rounded-xl shadow-[0_4px_0_rgb(22,163,74)] active:shadow-none active:translate-y-1 transition-all text-lg uppercase tracking-wide animate-bounce"
                    >
                      Collect Reward! 🎉
                    </button>
                  )}
                ...
                <div className="text-sm text-indigo-200 py-2 italic flex items-center gap-2">
                   Waiting for mission... ⏳
                </div>
              )
            ) : (
      ...
      <div className="flex flex-col-reverse md:flex-row md:justify-between md:items-end pointer-events-auto mt-auto gap-4 w-full max-w-full">
        ...
        <div className="w-full md:w-96 h-40 bg-orange-500 text-white rounded-3xl border-4 border-orange-400 shadow-[0_8px_0_rgb(234,88,12)] flex flex-col overflow-hidden relative">
          <div className="bg-orange-700/30 px-4 py-2 font-black text-sm uppercase tracking-wider flex justify-between items-center">
            <span>📰 City News</span>
          </div>
          ...
          <div ref={newsRef} className="flex-1 overflow-y-auto p-4 space-y-3 font-bold text-sm scroll-smooth">
            ...
            {newsFeed.map((news) => (
              <div key={news.id} className={`
                p-2 rounded-xl leading-tight relative shadow-sm border-2
                ${news.type === 'positive' ? 'bg-green-500 border-green-400' : ''}
                ${news.type === 'negative' ? 'bg-red-500 border-red-400' : ''}
                ${news.type === 'neutral' ? 'bg-blue-500 border-blue-400' : ''}
              `}>
                {news.text}
              </div>
            ))}
